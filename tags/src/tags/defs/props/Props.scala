...
trait Props {
  ...
  def prop[V, DomV](name: String, codec: Codec[V, DomV]): Prop[V, DomV] =
    new Prop(name, codec)

  @inline protected def boolProp(name: String): Prop[Boolean, Boolean] =
    prop(name, BooleanAsIsCodec)

  @inline protected def doubleProp(name: String): Prop[Double, Double] =
    prop(name, DoubleAsIsCodec)

  @inline protected def intProp(name: String): Prop[Int, Int] =
    prop(name, IntAsIsCodec)

  @inline protected def stringProp(name: String): Prop[String, String] =
    prop(name, StringAsIsCodec)

  // -- Props --

  /** In addition to the checked and unchecked states, there is a third state a
    * checkbox can be in: indeterminate. This is a state in which it's
  ...
  /** When the value of the type attribute is "radio" or "checkbox", this
    * property determines whether it is checked or not. This is different from
    * `checked` _attribute_, which contains the _initial_ checked status of the
    * element. More info: https://stackoverflow.com/a/6004028/2601788 (`checked`
    * behaves similar to `value`)
    *
    * See also: defaultChecked prop / attribute
    */
  lazy val checked: Prop[Boolean, Boolean] = boolProp("checked")

  /** Indicates whether an `<option>` element is _currently_ selected. This is
    * different from `selected` _attribute_, which contains the _initial_
    * selected status of the element. More info:
    * https://stackoverflow.com/a/6004028/2601788 (`selected` behaves similar to
    * `value`)
    *
  ...
  /** Current value of the element. This is different from `value` _attribute_,
    * which contains the _initial_ value of the element. More info:
  ...
  /** The URI of a program that processes the information submitted via the
    * form. This value can be overridden by a [[formAction]] attribute on a
  ...
  /** This is a nonstandard attribute used by Chrome and iOS Safari Mobile,
    * which controls whether and how the text value should be automatically
  ...
  /** The visible width of text input or `<textArea>`, in average character
    * widths. If it is specified, it must be a positive integer. If it is not
    * specified, the default value is 20 (HTML5).
    *
  ...
  /** This attribute contains a non-negative integer value that indicates for
    * how many columns the cell extends. Its default value is 1; if its value is
    * set to 0, it extends until the end of the `<colgroup>`, even if implicitly
    * defined, that the cell belongs to. Values higher than 1000 will be
  ...
  /** This attribute gives the value associated with the [[name]] or
    * [[httpEquiv]] attribute, of a `<meta>` element, depending on which of
  ...
  /** When the value of the type attribute is "radio" or "checkbox", the
    * presence of this Boolean attribute indicates that the control is selected
    * **by default**; otherwise it is ignored.
    *
    * See [[Props.checked]]
    *
  ...
  /** Indicates whether this `<option>` is initially selected in an option list
    * of a `<select>` element.
    *
    * See [[Props.selected]]
    *
  ...
  /** This Boolean attribute indicates that the form control is not available
    * for interaction. In particular, the click event will not be dispatched on
  ...
  /** Prompts the user to save the linked URL instead of navigating to it. Can
    ...
    *     - The media type (from the Content-Type header, the start of a data:
    *       URL, or Blob.type for a blob: URL)
    *   - Defining a value suggests it as the filename. / and \ characters are
    *     converted to underscores (_). Filesystems may forbid other characters
  ...
  /** Describes an element which belongs to this one. Used on labels and output
    * elements.
    *
    * [[https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/for for @ MDN]]
    */
  lazy val forId: Prop[String, String] = stringProp("for")

  /** The `formenctype` attribute provides the encoding type of the form when it
    * is submitted (for forms with a method of "POST") and overrides the default
    * `enctype` attribute of the `form` element. This should be used only with
    * the `input` elements of `type` "submit" or "image"
    *
    * Enumerated: "multipart/form-data" | "text/plain" |
  ...
  /** The `formmethod` attribute specifies the HTTP Method the form should use
    * when it is submitted and overrides the default `method` attribute of the
    * `form` element. This should be used only with the `input` elements of
    * `type` "submit" or "image".
    *
    * Enumerated: "post" | "get"
    *
  ...
  /** The `formnovalidate` Boolean attribute specifies that the input of the
    * form should not be validated upon submit and overrides the default
    * `novalidate` attribute of the `form`. This should only be used with
    * `input` elements of of `type` "submit".
    *
  ...
  /** The `formtarget` provides a name or keyword that indicates where to
    * display the response that is received after submitting the form and
    * overrides the `target` attribute of them `form` element. This should only
    * be used with the `input` elements of `type` "submit" or "image"
    *
  ...
  /** The inputmode attribute hints at the type of data that might be entered by
    ...
    *
    * Acceptable values: `none` | `text` (default value) | `decimal` | `numeric`
  ...
  /** For `optgroup` elements, specifies the name of the group of options, which
  ...
  /** Indicates how the browser should load the image:
    *
    * "eager": Loads the image immediately, regardless of whether or not the
    * image is currently within the visible viewport (this is the default
    * value).
    *
    * "lazy": Defers loading the image until it reaches a calculated distance
    ...
    * reasonably certain that it will be needed. This generally improves the
  ...
  /** If the type of the input element is text, email, search, password, tel, or
    ...
    * code points) that the user can enter. For other control types, it is
    * ignored.
    *
  ...
  /** The maximum allowed length for the input field. This attribute forces the
    * input control to accept no more than the allowed number of characters. It
    * does not produce any feedback to the user; you must write Javascript to
    * make that happen.
    *
  ...
  /** The HTTP method that the browser uses to submit the form. Possible values
    ...
    *     action attribute URI with a '?' as a separator, and the resulting URI
    *     is sent to the server. Use this method when the form has no
  ...
  /** This Boolean attribute specifies, when present/true, that the user is
    * allowed to enter more than one value for the `<input>` element for types
    * "email" or "file". It can also be provided to the `<select>` element to
  ...
  /** On form elements (input etc.): Name of the element. For example used by
    * the server to identify the fields in form submits.
    *
    * On the meta tag: This attribute defines the name of a document-level
    * metadata. This document-level metadata name is associated with a value,
  ...
  /** Specifies a regular expression to validate the input. The pattern
    * attribute works with the following input types: text, search, url, tel,
    * email, and password. Use the `title` attribute to describe the pattern to
    * the user.
    *
  ...
  /** This attribute contains a non-negative integer value that indicates for
    ...
    * `<tbody>`, `<tfoot>`, even if implicitly defined, that the cell belongs
  ...
  /** This integer attribute indicates if the element can take input focus (is
    ...
    * and if so, at what position. It can takes several values:
    *
  ...
  /** A name or keyword indicating where to display the response that is
    * received after submitting the form. In HTML 4, this is the name of, or a
    ...
    *     current frame or HTML5 parent browsing context of the current one. If
  ...
  /** This attribute contains a text representing advisory information related
    * to the element it belongs too. Such information can typically, but not
  ...
  /** This attribute specifies how the browser should handle cross-origin
    ...
    * be loaded when requested from a different domain, and how to handle
    * potential security issues, like CORS (Cross-Origin Resource Sharing)
    * policies. The value of this attribute determines whether the browser will
    * allow or block loading of the resource, helping to enhance web security.
    *
