...
import tags.setters.HtmlAttrSetter
import tags.setters.AriaAttrSetter
...
class SvgAttr[V](
  ...
  def qualifiedName: String =
    namespace.map(_ + ":" + domName).getOrElse(domName)
}

object SvgAttr:
  def namespaceUrl(namespace: String): String = namespace match
...
class AriaAttr[V](
  ...
  def :=(value: V): AriaAttrSetter[V] = AriaAttrSetter(this, value)
}
